...
export function PlayerBottomNav() {
  ...
  if (!showPlayerNavigation) return null;
  return <nav aria-label="Navegação principal" className="player-bottom-nav fixed inset-x-0 bottom-0 z-30 border-t border-white/[.06] bg-[#111923]/95 px-3 pb-[max(.65rem,env(safe-area-inset-bottom))] pt-2 backdrop-blur"><div className="mx-auto grid max-w-[640px] grid-cols-4">{items.map((item) => { const active = item.active(pathname); const Icon = item.icon; return <Link aria-current={active ? 'page' : undefined} className={`player-bottom-nav-item ${active ? 'is-active' : ''} ${item.featured ? 'is-featured' : ''}`} href={item.href} key={item.href}><span className="player-bottom-nav-indicator" /><Icon /><span>{item.label}</span></Link>; })}</div></nav>;
}

export function usePlayerBottomNavigation() {
  const { isLoading, profile, session } = useAuth();
  return !isLoading && Boolean(session) && profile?.role === 'player';
}
