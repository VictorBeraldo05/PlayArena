...
type ReservationIntent = { arena: string; court: string; courtId: string; sport: string; startAt: string; endAt: string; price: string; duration: string; customerName: string; customerPhone: string };
...
function ReservationPage() {
  ...
  const [intent] = useState<ReservationIntent>(() => {
    const fromSearch = { arena: params.get('arena') ?? '', court: params.get('court') ?? '', courtId: params.get('courtId') ?? '', sport: params.get('sport') ?? '', startAt: params.get('startAt') ?? '', endAt: params.get('endAt') ?? '', price: params.get('price') ?? '', duration: params.get('duration') ?? '60', customerName: params.get('name') ?? '', customerPhone: params.get('phone') ?? '' };
    return fromSearch.courtId ? fromSearch : storedIntent() ?? fromSearch;
  });
  const [visitorName, setVisitorName] = useState(intent.customerName);
  const [visitorPhone, setVisitorPhone] = useState(intent.customerPhone);
  ...
  const name = session ? profileName : visitorName.trim();
  const phone = session ? profilePhone : visitorPhone.trim();
  const profileComplete = Boolean(profileName && profilePhone);
  const profileError = Boolean(session && !isLoading && !profile);

  function savePendingIntent() { window.sessionStorage.setItem(PENDING_RESERVATION_KEY, JSON.stringify({ ...intent, customerName: visitorName.trim(), customerPhone: visitorPhone.trim() })); }
  ...
  return <main className="min-h-[100dvh] bg-[#080D14] pb-28 text-white"><div className="mx-auto max-w-[640px] px-4 py-5 sm:px-5"><header className="grid grid-cols-[44px_1fr_44px] items-center"><button aria-label="Voltar aos resultados" className="grid min-h-11 place-items-center rounded-xl text-2xl text-[#9DA7B3]" onClick={() => router.back()} type="button">‹</button><h1 className="text-center text-xl font-black">Pré-reserva</h1><span /></header><ReservationDetails intent={intent} /><form className="mt-6" noValidate onSubmit={(event) => void submit(event)}>{!session ? <VisitorData name={visitorName} phone={visitorPhone} onName={setVisitorName} onPhone={setVisitorPhone} /> : isLoading ? <ProfileSkeleton /> : profileError ? <section className="rounded-[20px] border border-[#FF4B4B]/25 bg-[#111923] p-5"><p className="font-bold text-[#FFB3B3]">Não foi possível carregar seus dados.</p><button className="mt-3 text-sm font-bold text-[#8FFF3C]" onClick={() => void refreshProfile()} type="button">Tentar novamente</button></section> : !profileComplete ? <section className="rounded-[20px] border border-[#8FFF3C]/20 bg-[#111923] p-5"><h2 className="text-lg font-black">Complete seus dados para continuar.</h2><button className="mt-4 font-bold text-[#8FFF3C]" onClick={completeProfile} type="button">Completar perfil</button></section> : <ProfileData name={profileName} phone={profilePhone} onEdit={() => router.push(`/player/perfil?returnTo=${encodeURIComponent('/reservar')}`)} />}{error ? <p className="mt-4 rounded-2xl border border-[#FF4B4B]/30 bg-[#FF4B4B]/10 p-4 text-sm font-semibold text-[#FFB3B3]">{error}</p> : null}{conflict ? <div className="mt-4 rounded-[18px] border border-[#FF4B4B]/30 bg-[#FF4B4B]/10 p-4"><p className="text-sm font-semibold text-[#FFB3B3]">Esse horário não está mais disponível.</p><button className="mt-3 font-bold text-[#8FFF3C]" onClick={() => router.push('/buscar/disponibilidade')} type="button">Escolher outro horário</button></div> : null}<div className="fixed inset-x-0 bottom-0 z-20 border-t border-white/10 bg-[#111923]/95 px-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-3 backdrop-blur"><div className="mx-auto max-w-[640px]">{session && !isLoading && !profileComplete ? <button className="min-h-14 w-full rounded-2xl bg-[#8FFF3C] px-5 font-black text-[#080D14]" onClick={completeProfile} type="button">Completar perfil</button> : <button className="min-h-14 w-full rounded-2xl bg-[#8FFF3C] px-5 font-black text-[#080D14] disabled:opacity-60" disabled={isSubmitting || conflict || (Boolean(session) && (isLoading || profileError)) || (!session && (!name || !phone))} type="submit">{isSubmitting ? 'Solicitando...' : 'Solicitar pré-reserva'}</button>}</div></div></form></div></main>;
}

function ReservationDetails({ intent }: { intent: ReservationIntent }) { return <section className="mt-6 overflow-hidden rounded-[20px] border border-white/10 bg-[#111923]"><div className="grid grid-cols-[112px_1fr]"><Image alt="Arena esportiva" className="h-full min-h-32 object-cover" height={128} src="/img/playarena-hero.png" width={112} /><div className="p-4"><h2 className="text-lg font-black">{intent.arena}</h2><p className="mt-1 text-sm text-[#9DA7B3]">{intent.sport} · {intent.court}</p><div className="mt-4 space-y-2 text-sm"><p>{friendlyDate(intent.startAt)}</p><p>{formatReservationTimeRange(intent.startAt, intent.endAt)} ({intent.duration} min)</p><p className="font-black">{formatCurrencyBRL(intent.price)}</p></div></div></div></section>; }
function VisitorData({ name, phone, onName, onPhone }: { name: string; phone: string; onName: (value: string) => void; onPhone: (value: string) => void }) { return <section className="rounded-[20px] border border-white/10 bg-[#111923] p-5"><h2 className="text-lg font-black">Seus dados</h2><label className="mt-5 block text-sm font-bold">Nome completo<input className="mt-2" onChange={(event) => onName(event.target.value)} required value={name} /></label><label className="mt-4 block text-sm font-bold">WhatsApp<input className="mt-2" inputMode="tel" onChange={(event) => onPhone(event.target.value)} required value={phone} /></label></section>; }
function ProfileData({ name, phone, onEdit }: { name: string; phone: string; onEdit: () => void }) { return <section className="rounded-[20px] border border-white/10 bg-[#111923] p-5"><div className="flex items-center justify-between gap-3"><h2 className="text-lg font-black">Seus dados</h2><button className="text-sm font-bold text-[#8FFF3C]" onClick={onEdit} type="button">Editar dados</button></div><div className="mt-4 space-y-2 text-sm"><p className="font-bold">{name}</p><p className="text-[#9DA7B3]">{formatPhoneBR(phone)}</p></div></section>; }
...
function ReservationTicket({ arena, court, date, time, duration, price, image, imageFailed, onImageError }: { arena: string; court: string; date: { main: string; sub: string }; time: string; duration: string; price: string | number; image?: string; imageFailed: boolean; onImageError: () => void }) { return <section className="success-reveal success-ticket mt-7 w-full overflow-hidden rounded-[24px] border border-white/[.08] bg-[#111923]/88 text-left backdrop-blur-md"><div className="grid h-[160px] grid-cols-[104px_1fr]"><div className="success-ticket-image relative">{image && !imageFailed ? <Image alt="Esporte da reserva" className="h-full w-full object-cover" fill onError={onImageError} sizes="104px" src={image} /> : <div className="success-ticket-fallback absolute inset-0" />}<div aria-hidden="true" className="success-ticket-image-overlay absolute inset-0" /></div><div className="flex min-w-0 flex-col p-4"><div><p className="truncate text-lg font-extrabold tracking-[-.03em]">{arena}</p><p className="mt-0.5 truncate text-sm text-[#AAB7C5]">{court}</p></div><div className="mt-auto grid grid-cols-3 divide-x divide-white/[.08]"><TicketMetric main={date.main} sub={date.sub} /><TicketMetric main={time} sub={duration} /><TicketMetric main={formatCurrencyBRL(price)} sub="Total" /></div></div></div></section>; }
...
function scheduleImage(sport: string) { const normalized = sport.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().replace(/[^a-z0-9]/g, ''); return ({ society: '/img/sports/schedule/society.jpg', beachtennis: '/img/sports/schedule/beach-tennis.jpg', futevolei: '/img/sports/schedule/futevolei.jpg', tenis: '/img/sports/schedule/tenis.jpg', volei: '/img/sports/schedule/volei.jpg' } as Record<string, string>)[normalized]; }
