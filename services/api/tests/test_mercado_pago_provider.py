...
def test_webhook_route_reads_dotted_query_and_case_insensitive_headers(
    create_client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
    caplog: pytest.LogCaptureFixture,
) -> None:
    ...
    def fake_process_webhook(_provider_name, _payload, _signature, **metadata):
...
def test_local_signature_diagnostic_uses_environment_only() -> None:
    ...
    script = Path(__file__).resolve().parents[1] / "scripts" / "diagnose_mercado_pago_webhook.py"
    environment = {
        **os.environ,
        "MP_WEBHOOK_URL": f"https://example.test/payments/webhooks/mercado-pago?data.id={ORDER_ID}&type=order",
        "MP_X_REQUEST_ID": request_id,
        "MP_X_SIGNATURE": signature,
        "MERCADO_PAGO_WEBHOOK_SECRET": WEBHOOK_SECRET,
    }
    environment.pop("MP_DATA_ID", None)
